...
- **Secrets in `process.env`.** Only `DATA_DIR`, `DATABASE_URL`, `TZ` are read. Steam API key and optional web-session refresh token live in the `SteamUser` row; GOG/Epic client secrets are the public launcher constants. No `runtimeConfig` migration needed, and the DB client runs outside Nitro (drizzle-kit, migrate CLI) so `useRuntimeConfig` would not fit anyway.
