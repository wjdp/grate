...
- `server/services/gameAggregates.ts` `refreshGameAggregates`.
- Characterisation tests for `server/services/games`, Steam and GOG services.
...
5. `server/services/games.ts` + pages: read `Game.playtimeMinutes`/`lastPlayedAt`; include `gogGame`; `getGamePlaytimes` returns either provider's history; `GameIcon`/art from `GogGame.*Url`; "Open in GOG" (`goggalaxy://openGameView/{id}`).
