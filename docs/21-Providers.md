...
A provider supplies some subset of: account identity, an owned-games library, playtime, art and store metadata. Since doc 19 a provider is one entry in `PROVIDER_JOBS` (`server/providers/jobs.ts`) implementing:
