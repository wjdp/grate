...
const sortedGames = computed(() => {
  return filteredGames.value?.sort((a, b) => {
    ...
    if (sort.value === 'playtime') return (b.steamGame?.playtimeForever ?? 0) - (a.steamGame?.playtimeForever ?? 0);
    return 0;
  });
});
...
const totalPlayedGames = computed(() => {
  return games.value?.filter((game) => game.steamGame?.playtimeForever > 0).length;
});
const totalUnplayedGames = computed(() => {
  return games.value?.filter((game) => game.steamGame?.playtimeForever === 0).length;
});
const totalRecentGames = computed(() => {
  return games.value?.filter((game) => game.steamGame?.playtime2weeks > 0).length;
});
</script>

<template>
  <div>
    <h1 class="mt-2 font-bold text-2xl">Games</h1>
    ...
    <ul v-if='sortedGames' class='grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
